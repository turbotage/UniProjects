...
def exp_diffusion_model():
    ...
    initial_guess = [1000,0.004]
    
    data = torch.from_numpy(project_to_vector(imgs, mask)).transpose(0,1).cuda()
    guess = gsolver.get_uniform_guess(data, initial_guess)
    ...
    model = gsolver.get_model(model_expr)
    
    guess_order = torch.diag(torch.tensor(initial_guess)).cuda()
    
    start = time.time() 

    found_params, conv_perc, iterations = gsolver.solve(model, dependent, data, guess, guess_order, 0.001, 1000)
