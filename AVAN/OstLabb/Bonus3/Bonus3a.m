...
%%%%%%%%% WALL SETTINGS %%%%%%%%
cornerEdge = [1.2591*10^(-8), 1.2591*10^(-8), 1.2591*10^(-8)];
...
for t = TIMES
    ...
    [particlePositions,particleVelocities,pressure] = update_coll(dt,particlePositions,particleVelocities,...
        particleRadius,particleMass,cornerEdge,[0,0,0]);
...
NkT = particleNumber*k*T
V = cornerEdge(1)*cornerEdge(2)*cornerEdge(3);
A = 2*cornerEdge(1)*cornerEdge(2) + 2*cornerEdge(1)*cornerEdge(3)+2*cornerEdge(2)*cornerEdge(3);
pV = V*totalPressure/(A*length(TIMES))
rel_diff = abs(pV-NkT)/pV
